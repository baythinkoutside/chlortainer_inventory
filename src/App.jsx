...
function CameraScanner({ onScan, onClose, hint="" }) {
  ...
  // Decode an image blob using BarcodeDetector (Chrome/Android) or ZXing fallback
  async function decodeImage(blob) {
    ...
    // Try BarcodeDetector first (native, fast — works on Android Chrome)
    ...
    // Fallback: ZXing decode from image
    if (!window.ZXing) {
  ...
  async function handleFileChange(e) {
    ...
    try {
      ...
      if (text) {
        ...
        setStatus("error");
        setErrMsg("No barcode found in that image. Try again — hold steady and ensure good lighting.");
      }
    } catch(e) {
      setStatus("error");
      setErrMsg("Could not read image. Please try again.");
    }
