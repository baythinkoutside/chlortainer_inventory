    ...
    async function start() {
      ...
      try {
        ...
        await reader.decodeFromVideoDevice(back?.deviceId, videoRef.current, (result)=>{
          ...
          if (text!==lastScan) { setLastScan(text); if(navigator.vibrate)navigator.vibrate(80); onScan(text); }
        });
      } catch(e) { if(!cancelled){setStatus("error");setErrMsg(e?.message||"Camera access denied.");} }
    }
    start();
